...
export default function AcceptInvitationPage({ params }: { params: { token: string } }) {
  ...
  const loadInvitation = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/invitations/${params.token}`);
  ...
  const handleAccept = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/invitations/${params.token}/accept`, {
        method: 'POST',
