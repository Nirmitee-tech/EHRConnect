...
export default function UserManagementPage() {
  ...
  const fetchUserContext = async () => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/auth/user-context?email=${encodeURIComponent(session?.user?.email || '')}`,
        {
          headers: {
  ...
  const loadUsersWithOrgId = async (orgId: string, userId: string) => {
    try {
      setLoading(true);
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/orgs/${orgId}/users`,
        {
          headers: {
            'x-org-id': orgId,
            'x-user-id': userId,
  ...
  const loadUsers = async () => {
    try {
      setLoading(true);
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/orgs/${session?.org_id}/users`,
        {
          headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const endpoint = mode === 'create'
        ? `${import.meta.env.VITE_API_URL}/api/orgs/${orgId}/users`
        : `${import.meta.env.VITE_API_URL}/api/orgs/${orgId}/invitations`;
