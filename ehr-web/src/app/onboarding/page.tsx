          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            <div className="md:col-span-2">
              <div className="bg-white rounded-2xl shadow-lg p-8 border border-gray-100">
                {currentStep === 0 && <WelcomeStep session={session} />}
                {currentStep === 1 && <LocationsStep formData={formData} setFormData={setFormData} session={session} />}
                {currentStep === 2 && <StaffStep formData={formData} setFormData={setFormData} session={session} />}
                {currentStep === 3 && <RolesStep formData={formData} setFormData={setFormData} session={session} />}
                {currentStep === 4 && <CompleteStep formData={formData} session={session} />}
...
// Step Components
function WelcomeStep({ session }: any) {
  return (
    <div className="text-center py-8">
      ...
      <div className="bg-blue-50 rounded-xl p-6 max-w-lg mx-auto">
        <p className="text-sm text-gray-700">
          <strong>Organization ID:</strong> {session?.org_id || 'Not assigned'}
        </p>
        {session?.org_slug && (
          <p className="text-sm text-gray-700 mt-2">
            <strong>Organization Slug:</strong> {session.org_slug}
          </p>
        )}
        <p className="text-sm text-gray-700 mt-2">
          <strong>Your Role:</strong> {session?.roles?.includes('ORG_OWNER') ? 'Organization Owner' : 'User'}
        </p>
...
function LocationsStep({ formData, setFormData, session }: any) {
  ...
  const handleAddLocation = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/orgs/${session.org_id}/locations`,
        {
          method: 'POST',
          headers: { 
...
function StaffStep({ formData, setFormData, session }: any) {
...
function CompleteStep({ formData, session }: any) {
  return (
    <div className="text-center py-8">
