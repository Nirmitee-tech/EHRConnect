...
import {
  ...
  CheckCircle, AlertTriangle, CreditCard, Send, RefreshCw, ArrowRight,
  Eye, Edit, AlertCircle, Phone, Mail, Calendar, Download, Filter
} from 'lucide-react'
...
// Action Items (NEW)
const actionItems = [
  { id: 1, priority: 'high', type: 'denial', title: 'Review 12 High-Value Denials', amount: 45000, dueDate: 'Today', count: 12 },
  { id: 2, priority: 'high', type: 'followup', title: 'Follow Up on 25 Pending Claims', amount: 67000, dueDate: 'Today', count: 25 },
  { id: 3, priority: 'medium', type: 'coding', title: 'Fix 8 Coding Errors', amount: 18000, dueDate: 'Tomorrow', count: 8 },
  { id: 4, priority: 'medium', type: 'aging', title: 'Contact 15 Patients (90+ Days)', amount: 32000, dueDate: '2 days', count: 15 },
]

// Recent Activity (NEW)
const recentActivity = [
  { id: 1, action: 'Claim Approved', patient: 'John Smith', amount: 2450, payer: 'Medicare', time: '5 min ago', status: 'success' },
  { id: 2, action: 'Claim Denied', patient: 'Sarah Johnson', amount: 1850, payer: 'Aetna', time: '12 min ago', status: 'danger' },
  { id: 3, action: 'Payment Received', patient: 'Mike Davis', amount: 3200, payer: 'UHC', time: '18 min ago', status: 'success' },
  { id: 4, action: 'Claim Submitted', patient: 'Lisa Anderson', amount: 2100, payer: 'BCBS', time: '25 min ago', status: 'info' },
]

interface RCMDashboardEnhancedProps {
  filters?: {
    dateRange: string;
    location: string;
    department: string;
...
export default function RCMDashboardEnhanced({ filters }: RCMDashboardEnhancedProps = {}) {
  return (
    <div className="space-y-4">
      {/* Alerts & Priorities Section (NEW) */}
      <div className="grid grid-cols-12 gap-3">
        <Card className="col-span-8 border-l-4 border-l-red-500 bg-red-50 border border-red-200 shadow-sm">
          <CardContent className="p-4">
            <div className="flex items-start gap-3">
              <AlertCircle className="h-5 w-5 text-red-600 mt-0.5 shrink-0" />
              <div className="flex-1">
                <h3 className="text-sm font-semibold text-red-900 mb-1">High Priority Actions Required</h3>
                <p className="text-xs text-red-700">
                  You have <span className="font-bold">37 items</span> requiring immediate attention worth{' '}
                  <span className="font-bold">$112,000</span>. Address today to maintain cash flow.
                </p>
              </div>
        ...
        <Card className="col-span-4 border-l-4 border-l-amber-500 bg-amber-50 border border-amber-200 shadow-sm">
          <CardContent className="p-4">
            <div className="flex items-start gap-3">
              <AlertTriangle className="h-5 w-5 text-amber-600 mt-0.5 shrink-0" />
              <div className="flex-1">
                <h3 className="text-sm font-semibold text-amber-900 mb-1">Aging A/R Alert</h3>
                <p className="text-xs text-amber-700">
                  <span className="font-bold">$83,000</span> in 90+ day bucket
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <div className="grid grid-cols-8 gap-3">
        <Card className="bg-gradient-to-br from-blue-600 to-blue-700 border-0 shadow-md hover:shadow-lg transition-all cursor-pointer">
          <CardContent className="p-3 text-white">
            ...
            <div className="text-xs opacity-75 mt-1">▲ 8.5% vs last month</div>
          </CardContent>
        </Card>

        <Card className="bg-gradient-to-br from-emerald-600 to-emerald-700 border-0 shadow-md hover:shadow-lg transition-all cursor-pointer">
          <CardContent className="p-3 text-white">
            <div className="flex items-center justify-between mb-2">
            ...
            <div className="text-xs opacity-75 mt-1">Above benchmark ✓</div>
          </CardContent>
        </Card>

        <Card className="bg-gradient-to-br from-amber-600 to-amber-700 border-0 shadow-md hover:shadow-lg transition-all cursor-pointer">
          <CardContent className="p-3 text-white">
            <div className="flex items-center justify-between mb-2">
            ...
            <div className="text-xs opacity-75 mt-1">▼ 2.3 days improved</div>
          </CardContent>
        </Card>

        <Card className="bg-gradient-to-br from-rose-600 to-rose-700 border-0 shadow-md hover:shadow-lg transition-all cursor-pointer">
          <CardContent className="p-3 text-white">
            <div className="flex items-center justify-between mb-2">
            ...
            <div className="text-xs opacity-75 mt-1">35 claims to review</div>
          </CardContent>
        </Card>

        <Card className="bg-gradient-to-br from-violet-600 to-violet-700 border-0 shadow-md hover:shadow-lg transition-all cursor-pointer">
          <CardContent className="p-3 text-white">
            <div className="flex items-center justify-between mb-2">
            ...
            <div className="text-xs opacity-75 mt-1">89 ready to bill</div>
          </CardContent>
        </Card>

        <Card className="bg-gradient-to-br from-cyan-600 to-cyan-700 border-0 shadow-md hover:shadow-lg transition-all cursor-pointer">
          <CardContent className="p-3 text-white">
            <div className="flex items-center justify-between mb-2">
            ...
            <div className="text-xs opacity-75 mt-1">Industry: 95%</div>
          </CardContent>
        </Card>

        <Card className="bg-gradient-to-br from-indigo-600 to-indigo-700 border-0 shadow-md hover:shadow-lg transition-all cursor-pointer">
          <CardContent className="p-3 text-white">
            <div className="flex items-center justify-between mb-2">
            ...
            <div className="text-xs opacity-75 mt-1">15 need review</div>
          </CardContent>
        </Card>

        <Card className="bg-gradient-to-br from-teal-600 to-teal-700 border-0 shadow-md hover:shadow-lg transition-all cursor-pointer">
          <CardContent className="p-3 text-white">
            <div className="flex items-center justify-between mb-2">
            ...
            <div className="text-xs opacity-75 mt-1">$245K value</div>
          </CardContent>
        </Card>
      </div>

      {/* Action Items & Recent Activity (NEW) */}
      <div className="grid grid-cols-12 gap-3">
        <Card className="col-span-7 border border-gray-200 shadow-sm">
          <CardHeader className="pb-2 pt-3 px-4">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm font-semibold text-gray-900">Priority Action Items</CardTitle>
                ...
                >
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 text-xs text-gray-600">
                      <span className="font-medium text-gray-900">${(item.amount/1000).toFixed(0)}K</span>
                      <span>•</span>
                ...
                <div key={activity.id} className="flex items-start gap-3 p-2 rounded hover:bg-gray-50">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-0.5">
                      <span className="text-xs font-semibold text-gray-900">{activity.action}</span>
                      <span className="text-xs text-gray-500">{activity.time}</span>
                    </div>
                    <p className="text-xs text-gray-600 truncate">{activity.patient}</p>
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-xs text-gray-900 font-medium">${activity.amount.toLocaleString()}</span>
                      <span className="text-xs text-gray-500">•</span>
                      <span className="text-xs text-gray-600">{activity.payer}</span>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <CardHeader className="pb-2 pt-3 px-4">
            <div className="flex items-center justify-between">
              ...
              <Button size="sm" variant="ghost" className="h-7 text-xs text-blue-600">
                Fix All <ArrowRight className="h-3 w-3 ml-1" />
              </Button>
            </div>
          </CardHeader>
          <CardContent className="p-4 pt-2">
