...
import { Stethoscope, Users, CheckCircle, AlertTriangle, DollarSign } from 'lucide-react';
...
export default function ReportPage() {
  ...
  const controlledCount = reportData.reduce((sum, d) => sum + (d.controlledCount || Math.floor((d.patientCount || 0) * 0.6)), 0);
  const highRiskCount = reportData.filter(d => d.riskLevel === 'High' || d.riskLevel === 'Very High').length;
  const avgCost = '$' + (reportData.reduce((sum, d) => sum + (d.averageCost || 0), 0) / reportData.length / 1000).toFixed(1) + 'K';
    ...
    >
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Card key="1">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-medium text-gray-600">Controlled</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-2xl font-bold text-gray-900">{controlledCount}</div>
                </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-center py-12 text-gray-500">
              ...
              <div className="text-xs mt-2">Charts, graphs, and detailed insights for Chronic Disease Management</div>
            </div>
          </CardContent>
        </Card>
      </div>
