...
import { Syringe, Send, CheckCircle, Users } from 'lucide-react';
...
export default function ReportPage() {
  ...
  const submissionCount = Math.floor(reportData.length * 0.85);
  const complianceRate = ((reportData.filter(d => d.status === 'Compliant').length / reportData.length) * 100).toFixed(0);
  const uniquePatients = new Set(reportData.map(d => d.patientName)).size;
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card key="0">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-medium text-gray-600">Vaccines Given</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card key="1">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-medium text-gray-600">Registry Submissions</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-center justify-between">
                <div>
          ...
          <Card key="3">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-medium text-gray-600">Patients Immunized</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-center justify-between">
                <div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-center py-12 text-gray-500">
              ...
              <div className="text-xs mt-2">Charts, graphs, and detailed insights for Immunization Registry</div>
            </div>
          </CardContent>
        </Card>
      </div>
