...
// Export for use in Sequelize seeders
async function seedInventoryMasters(externalPool) {
  const dbPool = externalPool || pool;

  console.log('🌱 Starting inventory masters seed...\n');

  try {
    // Create temporary functions that use the external pool
    const seedLocationsWithPool = async () => {
      console.log('🏥 Seeding default locations...');

      try {
        const { rows: orgs } = await dbPool.query('SELECT id, name FROM organizations');
    ...
    await seedLocationsWithPool();
    // Note: Categories and suppliers are already seeded by initial seed

    console.log('✨ Inventory masters seeded successfully!');
  } catch (error) {
    console.error('❌ Seeding failed:', error);
    throw error;
  }
...
// Export for Sequelize seeders
module.exports = { seedInventoryMasters };
