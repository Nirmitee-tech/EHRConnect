...
class FormsService {
  ...
  /**
   * Create or update a step validation rule
   * @param {string} stepId - Step UUID
   * @param {object} ruleData - Rule definition { type, conditions, actions, enabled }
   * @param {string} userId - User creating/updating the rule
   * @param {string} orgId - Organization ID
   * @returns {object} Updated step with new rule
   */
  async createStepRule(stepId, ruleData, userId, orgId) {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      ...
      const validationConfig = step.validation_config || {};
      ...
      const newRule = {
        ...
        type: ruleData.type || 'validation', // validation | visibility | navigation
      ...
      // Add rule to validation_config
      validationConfig.rules.push(newRule);

      // Update step
      const updateResult = await client.query(
        `UPDATE form_steps
         SET validation_config = $1, updated_by = $2, updated_at = CURRENT_TIMESTAMP
         WHERE id = $3 AND org_id = $4
         RETURNING *`,
        [validationConfig, userId, stepId, orgId]
      );

      // Audit log
      await this.logAudit(client, {
        org_id: orgId,
        entity_type: 'form_step_rule',
        entity_id: newRule.id,
        action: 'created',
        actor_id: userId,
        metadata: { step_id: stepId, rule_type: ruleData.type }
      });

      await client.query('COMMIT');
      return { step: updateResult.rows[0], rule: newRule };
    } catch (error) {
      await client.query('ROLLBACK');
   ...
   */
  async getStepRules(stepId, orgId) {
    const query = `
      SELECT validation_config, conditional_logic
      FROM form_steps
      WHERE id = $1 AND org_id = $2
    `;
    const result = await pool.query(query, [stepId, orgId]);
    ...
    // Also include conditional_logic as a rule if it exists
    if (step.conditional_logic) {
      rules.push({
        ...
        enabled: true,
        conditions: step.conditional_logic.conditions || [],
        actions: [{ type: 'show_hide', value: 'show' }],
        isConditionalLogic: true
      });
    }
   ...
   * @param {string} orgId - Organization ID
   * @returns {object} Evaluation result { valid, errors, warnings, visibilityAllowed }
   */
  async evaluateStepRules(stepId, context, orgId) {
    const rules = await this.getStepRules(stepId, orgId);
    ...
    for (const rule of rules) {
      if (!rule.enabled && !rule.isConditionalLogic) continue;
      ...
      // Evaluate conditions
      ruleResult.conditionsMet = this.evaluateConditions(rule.conditions, context);

      // If conditions are met, process actions
      if (ruleResult.conditionsMet) {
        for (const action of rule.actions) {
          const actionResult = this.processRuleAction(action, rule, context);
          ...
          } else if (action.type === 'show_hide') {
            result.visibilityAllowed = actionResult.value === 'show';
          }
        }
      }

      result.evaluatedRules.push(ruleResult);
    }
  ...
  evaluateConditions(conditions, context) {
    ...
    // Support for grouped conditions with AND/OR logic
    const logicOperator = conditions[0]?.logicOperator || 'AND';
    ...
    const results = conditions.map(condition => {
      ...
      switch (operator) {
        ...
        case 'less_or_equal':
          return Number(contextValue) <= Number(value);
        case 'is_empty':
          return !contextValue || contextValue === '' || (Array.isArray(contextValue) && contextValue.length === 0);
        case 'is_not_empty':
          return contextValue && contextValue !== '' && (!Array.isArray(contextValue) || contextValue.length > 0);
        case 'in':
   ...
   */
  processRuleAction(action, rule, context) {
    const result = {
    ...
    switch (action.type) {
      case 'error':
        result.message = this.interpolateMessage(action.message || 'Validation error', context);
        break;
      case 'warning':
        result.message = this.interpolateMessage(action.message || 'Warning', context);
        break;
      case 'show_hide':
        result.value = action.value; // 'show' or 'hide'
        break;
      case 'set_value':
        result.field = action.field;
        result.value = this.interpolateValue(action.value, context);
        break;
      case 'enable_disable':
        result.field = action.field;
        result.value = action.value; // 'enable' or 'disable'
        break;
      default:
   ...
   * @param {string} path - Dot notation path (e.g., 'patient.age')
   ...
   * @private
   * @param {string} message - Message template with {{field}} placeholders
   * @param {object} context - Context data
  ...
  /**
   * Interpolate value with context data
   * @private
   ...
   * @param {object} mockData - Mock context data
   * @param {string} orgId - Organization ID
   * @returns {object} Test result
   */
  async testStepRule(stepId, ruleId, mockData, orgId) {
    const rules = await this.getStepRules(stepId, orgId);
    ...
    if (rule.conditions) {
      testResult.conditionsEvaluation = rule.conditions.map(condition => {
        const contextValue = this.getNestedValue(mockData, condition.field);
        const result = this.evaluateSingleCondition(condition, contextValue);
        return {
          condition,
          contextValue,
          result,
          explanation: `${condition.field} ${condition.operator} ${condition.value} => ${result}`
        };
      });

      testResult.conditionsMet = this.evaluateConditions(rule.conditions, mockData);
    }

    // If conditions met, show what actions would execute
    if (testResult.conditionsMet) {
      testResult.actionsExecuted = rule.actions.map(action => {
        return this.processRuleAction(action, rule, mockData);
      });
    }
  ...
  /**
   * Evaluate single condition for debugging
   * @private
   * @param {object} condition - Condition object
   * @param {any} contextValue - Actual context value
   * @returns {boolean} Condition result
   */
  evaluateSingleCondition(condition, contextValue) {
   ...
   * @param {string} userId - User updating the rule
   * @param {string} orgId - Organization ID
   * @returns {object} Updated step with modified rule
   */
  async updateStepRule(stepId, ruleId, ruleData, userId, orgId) {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      ...
      await this.logAudit(client, {
        ...
        action: 'updated',
        actor_id: userId,
        metadata: { step_id: stepId, changes: ruleData }
      });

      await client.query('COMMIT');
      return { step: updateResult.rows[0], rule: rules[ruleIndex] };
    } catch (error) {
      await client.query('ROLLBACK');
   ...
   * @param {string} userId - User deleting the rule
   * @param {string} orgId - Organization ID
   * @returns {object} Updated step without the deleted rule
   */
  async deleteStepRule(stepId, ruleId, userId, orgId) {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      ...
      const step = stepResult.rows[0];
      const validationConfig = step.validation_config || {};
      let rules = validationConfig.rules || [];

      // Find rule to confirm it exists
